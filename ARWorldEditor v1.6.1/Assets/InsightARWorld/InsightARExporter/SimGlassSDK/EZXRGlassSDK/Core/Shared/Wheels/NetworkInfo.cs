...
namespace Wheels.Unity
{
    public class NetworkInfo
    {
        ...
        {
            IPAddress[] tempIPA = Dns.GetHostAddresses(Dns.GetHostName());
            List<string> ipv4 = new List<string>();
            foreach (IPAddress itm in tempIPA)
            {
                if (itm.AddressFamily == AddressFamily.InterNetwork)// && itm.ToString().Contains("192.168."))
                {
                    ipv4.Add(itm.ToString());
                }
            }

            string[] ipv4Array = ipv4.ToArray();
